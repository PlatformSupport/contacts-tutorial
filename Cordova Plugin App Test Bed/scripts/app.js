(function () {
	...
	window.getAllContacts = function () {
		...
		options.filter = ""; // "" returns all contacts
	...
	window.getContactDetails = function (e) {
		selectedContactId = e.view.params.rawId;
		var options = new ContactFindOptions();
		options.filter = "";
		options.multiple = true;
		var fields = ["*"];
		...
		for (var i = 0; i < contacts.length; i++) {
			if (contacts[i].id == selectedContactId) {
				...
				if (contacts[i].phoneNumbers) {
					...
					$("#contact-phone").text("No phone numbers for this contact entry");
				}
				// added in step 5 - we need this to set the large picture to the contact details view
